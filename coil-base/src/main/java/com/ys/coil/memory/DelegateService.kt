...
internal class DelegateService(
    ...
    fun createTargetDelegate(request: Request): TargetDelegate {
        ...
        return when {
            request is Request.GetRequest -> InvalidatableEmptyTargetDelegate(referenceCounter)
    ...
    fun createRequestDelegate(
        ...
        val requestDelegate = when (request) {
            is Request.GetRequest -> EmptyRequestDelegate
            is Request.LoadRequest -> if (request.target is ViewTarget<*>) {
