...
class WeakMemoryCacheTest {
	...
	private lateinit var reference: MutableSet<Any>

	@Before
	fun before() {
		context = ApplicationProvider.getApplicationContext()
		weakMemoryCache = RealWeakMemoryCache()
		reference = arraySetOf()
	}

	@Test
	fun `can retrieve cache value`() {
	...
	/**
	 * [RealWeakMemoryCache.cache]에서 항목을 제거하지 않고 [bitmap]의 약한 참조를 지웁니다.
	 * 이것은 가비지 수집을 시뮬레이션합니다.
	 */
	...
	/**
	 * 가비지 수집을 방지하기 위해 테스트 동안 값에 대한 강력한 참조를 유지합니다.
	 */
